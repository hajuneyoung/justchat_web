                    ...
                    <div class="row no-gutters">
                        <div class="col-xl-4 col-lg-4 col-md-4 col-sm-3 col-3">
                            <div class="users-container">
                                <div id="memberNumber">현재 인원 : <%=memberNumber %></div>
                        ...
                        <div class="col-xl-8 col-lg-8 col-md-8 col-sm-9 col-9">
                            ...
                            <div class="chat-container">
                            	<div class="form-group mt-3 mb-0">
                                    <input type="text" class="form-control" placeholder="메세지를 입력해주세요...귓속말 명령어는/w 닉네임 귓속말내용"
...
<script>
var ws1 
= new WebSocket("ws://localhost:8080"+"<%=request.getContextPath()%>/roomChat"); 
 		          
<%-- var ws1=new WebSoket("ws://localhost:8080"+"<%=request.getContextPath()%>/kaja"); --%> 
...
ws1.onopen = function(aa){
	...
	ws1.send("clientInfo/<%=clientInfo%>");
};
...
ws1.onmessage = function(aa){
	if(/^\[.* 님에게 귓속말\].*/.test(aa.data)){//귓속말
	    ...
	    document.getElementById("whisperBox").appendChild(liChat);
	    if(document.getElementById("autoScroll").checked){
	    	document.getElementById("whisperContainer").scrollTop = 
	    		document.getElementById("whisperContainer").scrollHeight;
	    }
	}else if(/^\[.* 님으로부터 귓속말\].*/.test(aa.data)){
	    ...
	    document.getElementById("whisperBox").appendChild(li);
	    if(document.getElementById("autoScroll").checked){
	    	document.getElementById("whisperContainer").scrollTop = 
	    		document.getElementById("whisperContainer").scrollHeight;
	    }
	}else if(/^\[.*\].*/.test(aa.data)){//일반메세지
	...
	}else if(/^add\/.*\/.*/.test(aa.data)){
		var arr = aa.data.split("/",3);
		document.getElementById("memberNumber").innerHTML = '현재 인원 : '+arr[1];
		var ul = document.createElement( "ul" );
		ul.setAttribute('id','member'+arr[2]);
		ul.setAttribute('class','person');
	...
	}else if(/^remove\/.*\/.*/.test(aa.data)){
		var arr = aa.data.split("/",3);
		document.getElementById("memberNumber").innerHTML = '현재 인원 : '+arr[1];
		document.getElementById('member'+arr[2]).remove();
	}
	else{
		var li = document.createElement( "li" );
...
function kajaChool(){
	...
	if(/\/w .* .*/.test(chat1.value)){
		if(chat1.value.split(" ",3)[1]=='<%=userNickName%>') {
			var li = document.createElement( "li" );
			li.setAttribute('class','person');
			var p = document.createElement( "p" );
			p.setAttribute('class','name-time');
			var span = document.createElement( "span" );
			span.setAttribute('class','time');
			span.innerHTML = "자신에게 귓속말을 보낼 수 없습니다";
			p.appendChild(span);
			li.appendChild(p);
			document.getElementById("notice").appendChild(li);
			if(document.getElementById("autoScroll").checked){
		...
		ws1.send("whisper/<%=userNickName%>/"+chat1.value.split(" ",3)[1]+"/"+chat1.value.split(" ",3)[2]);
		chat1.value = "";
		chat1.focus();
	}else{
